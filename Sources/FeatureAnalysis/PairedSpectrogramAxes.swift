...
public struct PairedSpectrogramAxes: Equatable {

    /// Which of the two files a lane belongs to. Position, and nothing more.
    public typealias Side = PairedWaveformAxis.Side
    ...
    public struct Lane: Equatable {
        ...
        public let frequencyFraction: Double
        ...
        public var outOfRangeFraction: Double { 1 - frequencyFraction }
    }

    /// The frequency extent both lanes are measured against: the higher of the two Nyquists.
    public let sharedNyquist: Double
    ...
    public func occupiedRect(_ side: Side, in size: CGSize) -> CGRect? {
    ...
    public func outOfRangeRect(_ side: Side, in size: CGSize) -> CGRect? {
        guard let lane = lane(side) else { return nil }
        return CGRect(
            x: 0,
            y: 0,
    ...
    public static let outOfRangeTreatment: (red: Double, green: Double, blue: Double) = (0.32, 0.32, 0.32)
}
