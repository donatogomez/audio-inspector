...
public final class ImportFlowModel {
    ...
    private func settle(
        ...
        switch outcome {
        case let .inspected(report, _, _, _, _, _):
    ...
    private func apply(_ outcome: SourceInspectionOutcome, restoringOnCancellation previous: State) {
        switch outcome {
        case let .inspected(report, waveform, spectrogram, signalLevelMetrics, truePeak, loudness):
            ...
            let presentation = InspectionPresentation(
                ...
                loudness: LoudnessState(loudness) ?? .unavailable
            )
