...
struct ImportFlowModelTests {
    ...
    @Test func cancellingAfterAReportKeepsThePreviousReport() async {
        ...
        let sequence = SequenceAction([.inspected(first, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable), .cancelled])
        ...
        #expect(model.state == .report(InspectionPresentation(report: first, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable))) // exactly where they were, no error
    }
    ...
    @Test func aFailedReportIsPresentedAsAReportNotAsAFlowError() async {
        ...
        let model = ImportFlowModel(action: ScriptedAction(.inspected(failedReport, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable)).run)

        await model.selectAndInspect()

        // The domain already models this; the flow must not convert it into its own `failed` state.
        #expect(model.state == .report(InspectionPresentation(report: failedReport, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable)))
    }
    ...
    @Test func aSecondSelectionIsIgnoredWhileOneIsInFlight() async {
        ...
        action.finish(.inspected(report, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable))
        await first.value
        #expect(model.state == .report(InspectionPresentation(report: report, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable)))
    }
    ...
    @Test func aSecondSelectionReplacesThePreviousReport() async {
        ...
        let model = ImportFlowModel(action: SequenceAction([.inspected(first, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable), .inspected(second, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable)]).run)
    ...
    @Test func selectingAgainAfterAPreparationFailureCanSucceed() async {
        let report = makeReport(named: "clip.wav")
        let model = ImportFlowModel(action: SequenceAction([.preparationFailed, .inspected(report, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable)]).run)
