...
struct ImportFlowComparisonTests {
    ...
    func theSecondFilesVisualisationsChangeNothing() async {
        ...
        second.finish(
            ...
            .inspected(other, waveform: .unavailable, spectrogram: .failed(message: "no"), signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable)
        )
        await comparing.value
    ...
    func aGloballyFailedSecondFileStillCompares() async {
        ...
        let comparing = Task { await flow.compare(using: second.run) }
        await second.waitUntilStarted()
        second.finish(.inspected(broken, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable))
        await comparing.value
    ...
    func aThirdFileReplacesTheSecondOnly() async {
        ...
        bAction.finish(
            sending: [.report(b)],
            .inspected(b, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable)
        )
    ...
    func aNewPrimaryInspectionEndsTheComparison() async {
        ...
        fresh.finish(.inspected(newPrimary, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable))
    ...
    func consecutiveComparisonsShareNoState() async {
        ...
        firstAction.finish(.inspected(b, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable))
        await first.value
        ...
        secondAction.finish(
            sending: [.report(c)],
            .inspected(c, waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable)
        )
        await second.value
    ...
    func aComparisonNeverWarnsAboutTheFirstFile() async {
        ...
        let outcomes: [SourceInspectionOutcome] = [
            .cancelled,
            .preparationFailed,
            .inspected(globallyFailedReport(named: "b.wav"), waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable),
        ]
