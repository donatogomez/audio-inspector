...
struct ExportComparisonIsolationTests {
    ...
    private func reportHeldByTheFlow(
        ...
        if let second {
            ...
            secondAction.finish(.inspected(second, analyses: InspectionAnalyses(waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
            await comparing.value
        }

        action.finish(.inspected(primary, analyses: InspectionAnalyses(waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
        await running.value
