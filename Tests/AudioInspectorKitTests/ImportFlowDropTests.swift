...
struct ImportFlowDropTests {
    ...
    @Test func aDroppedSourceReplacesAnExistingReportWhenItCompletes() async {
        ...
        let model = ImportFlowModel(action: ScriptedAction(.inspected(first)).run)

        await model.selectAndInspect()
        #expect(model.state == .report(first))

        await model.inspectDroppedSource(using: ScriptedAction(.inspected(second)).run)
    ...
    @Test func aGlobalFailureFromADropStaysAReport() async {
        ...
        await model.inspectDroppedSource(using: ScriptedAction(.inspected(failed)).run)

        // The domain models a global failure; the flow must not convert it into its own `failed`.
        #expect(model.state == .report(failed))
    }
    ...
    @Test func aDropIsIgnoredWhileAnInspectionIsInFlight() async {
        ...
        running.finish(.inspected(report))
        await first.value
        #expect(model.state == .report(report))
    }
    ...
    func aRejectionPreservesAnExistingReport(_ rejection: DropRejection) async {
        ...
        let model = ImportFlowModel(action: ScriptedAction(.inspected(report)).run)

        await model.selectAndInspect()
        model.reject(rejection)

        #expect(model.state == .report(report)) // the report survives the refusal
        #expect(model.dropRejection == rejection)
    }
    ...
    @Test func avalidDropClearsThePendingRejection() async {
        ...
        await model.inspectDroppedSource(using: ScriptedAction(.inspected(report)).run)
