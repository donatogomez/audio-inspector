...
struct DroppedSourceInspectionTests {
    ...
    @Test func aDroppedFixtureIsInspectedThroughTheRealPipeline() async throws {
        try await withTemporaryDirectory { directory in
            ...
            guard case let .inspected(report, _, _, _, _, _) = await coordinator.inspect(accepted, onUpdate: { _ in }) else {
...
let report = presentation.report
    ...
    @Test func thePanelAndTheDropProduceTheSameReportForTheSameFile() async throws {
        try await withTemporaryDirectory { directory in
            ...
            guard case let .inspected(panelReport, _, _, _, _, _) = await viaPanel.inspect(onUpdate: { _ in }),
                  case let .inspected(dropReport, _, _, _, _, _) = await viaDrop.inspect(url, onUpdate: { _ in })
            else {
