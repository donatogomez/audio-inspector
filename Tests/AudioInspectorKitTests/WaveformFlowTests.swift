...
private final class SuspendedAction {
    ...
    private func observeCancellation() {
        ...
        if finishesOnCancellation {
            finish(.inspected(report, waveform: .cancelled))
        }
    }
...
struct WaveformCoordinationTests {
    ...
    func realFileProducesBoth() async throws {
        try await withTemporaryDirectory { directory in
            ...
            let outcome = await coordinator.inspect(url, onReport: { delivered = $0 })
    ...
    func generatorReceivesTheSameReference() async throws {
        try await withTemporaryDirectory { directory in
            ...
            var delivered: InspectionReport?
            _ = await coordinator.inspect(url, onReport: { delivered = $0 })
    ...
    func scopeCoversTheWaveform() async throws {
        try await withTemporaryDirectory { directory in
            ...
            guard case let .inspected(_, waveform) = outcome, case .available = waveform else {
    ...
    func sourceIsUnchanged() async throws {
        try await withTemporaryDirectory { directory in
            ...
            _ = await SourceInspectionCoordinator().inspect(url, onReport: { _ in })
...
struct WaveformFlowStateTests {
    ...
    func reportAppearsBeforeTheWaveform() async throws {
        ...
        action.finish(.inspected(action.report, waveform: .available(envelope())))
        await running.value

        #expect(model.state == .report(InspectionPresentation(report: action.report, waveform: .available(envelope()))))
    }
    ...
    func unavailableReplacesLoading() async throws {
        ...
        action.deliverReport()
        await Task.yield()
        action.finish(.inspected(action.report, waveform: .unavailable))
        await running.value

        #expect(model.state == .report(InspectionPresentation(report: action.report, waveform: .unavailable)))
    }
    ...
    func failedWaveformKeepsTheReport() async throws {
        ...
        action.deliverReport()
        await Task.yield()
        action.finish(.inspected(action.report, waveform: .failed(message: "The waveform could not be produced.")))
        await running.value
    ...
    func selectionDuringInspectionIsIgnored() async throws {
        ...
        first.deliverReport()
        first.finish(.inspected(first.report, waveform: .unavailable))
        await running.value
    ...
    func selectionDuringWaveformSupersedes() async throws {
        ...
        await Task.yield()
        second.finish(.inspected(second.report, waveform: .available(envelope())))
    ...
    func staleResultsAreDropped() async throws {
        ...
        second.deliverReport()
        await Task.yield()
        second.finish(.inspected(second.report, waveform: .unavailable))
        await secondRun.value

        // The superseded operation now tries to report a completely different result. It must not land.
        first.finish(.inspected(first.report, waveform: .available(envelope())))
        _ = await firstRun.value
