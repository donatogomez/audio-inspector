...
private final class SuspendedAction {
    ...
    private func observeCancellation() {
        ...
        if finishesOnCancellation {
            finish(.inspected(report, waveform: .cancelled, spectrogram: .unavailable))
        }
    }
...
struct WaveformCoordinationTests {
    ...
    func scopeCoversTheWaveform() async throws {
        try await withTemporaryDirectory { directory in
            ...
            guard case let .inspected(_, waveform, _) = outcome, case .available = waveform else {
...
struct WaveformFlowStateTests {
    ...
    func reportAppearsBeforeTheWaveform() async throws {
        ...
        action.finish(.inspected(action.report, waveform: .available(envelope()), spectrogram: .unavailable))
        await running.value
    ...
    func failedWaveformKeepsTheReport() async throws {
        ...
        action.deliverReport()
        await Task.yield()
        action.finish(.inspected(action.report, waveform: .failed(message: "The waveform could not be produced."), spectrogram: .unavailable))
        await running.value
    ...
    func selectionDuringWaveformSupersedes() async throws {
        ...
        await Task.yield()
        second.finish(.inspected(second.report, waveform: .available(envelope()), spectrogram: .unavailable))
    ...
    func staleResultsAreDropped() async throws {
        ...
        first.finish(.inspected(first.report, waveform: .available(envelope()), spectrogram: .unavailable))
        _ = await firstRun.value
