...
struct SignalLevelMetricsPresentationTests {
    ...
    private func stereoMetrics() throws -> SignalLevelMetrics {
        let left = try channel(peak: 0.708, rms: 0.3, dcOffset: 0.002, clipped: 3)
        let right = try channel(peak: 0.5, rms: 0.2, dcOffset: -0.001, clipped: 0)
        return try metrics(
    ...
    private func zeroFrameMetrics() throws -> SignalLevelMetrics {
        ...
        return try metrics(channels: [empty], overallPeak: nil, overallRMS: nil, overallDCOffset: nil, overallClipped: 0)
    }

    /// Real, computed silence — distinct from `try zeroFrameMetrics()`: `sampleCount > 0`, every value a
    /// genuine zero rather than absent.
    private func silentMetrics() throws -> SignalLevelMetrics {
        let silent = try channel(peak: 0, rms: 0, dcOffset: 0, clipped: 0)
        return try metrics(channels: [silent], overallPeak: 0, overallRMS: 0, overallDCOffset: 0, overallClipped: 0)
    }

    // MARK: - Rows: values and units

    @Test func peakAndRMSReadInDecibelsFullScale() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try monoMetrics())
    ...
    @Test func dcOffsetReadsAsALinearSignedValueNeverInDecibels() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try monoMetrics())
    ...
    @Test func clippedSamplesReadAsAPlainCount() throws {
        let many = try metrics(channels: [try channel(clipped: 12_431)], overallPeak: 0.1, overallRMS: 0.05, overallDCOffset: 0, overallClipped: 12_431)
    ...
    @Test func aRealZeroClipCountIsShownAsZeroNeverAsAbsent() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try monoMetrics())
        let clipped = try! #require(rows.first { $0.name == "Clipped samples" })
    ...
    @Test func zeroFramesReportsNotComputableRatherThanZero() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try zeroFrameMetrics())
    ...
    @Test func realSilenceIsAComputedValueNotAnAbsence() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try silentMetrics())
        let peak = try! #require(rows.first { $0.name == "Peak sample" })
    ...
    @Test func clipCountIsAlwaysDefinedEvenForZeroFrames() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try zeroFrameMetrics())
        let clipped = try! #require(rows.first { $0.name == "Clipped samples" })
        #expect(clipped.value == "0")
    ...
    /// channel's figure.
    @Test func aSingleChannelShowsNoPerChannelBreakdown() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try monoMetrics())
        for row in rows {
    ...
    @Test func multipleChannelsShowThePerChannelBreakdown() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try stereoMetrics())
        let peak = try! #require(rows.first { $0.name == "Peak sample" })
    ...
    @Test func channelsAreNeverNamedByAssumedLayout() throws {
        let rows = SignalLevelMetricsCopy.rows(for: try stereoMetrics())
        for row in rows {
    ...
    /// named.
    @Test func multichannelStaysLegibleAndNumbersEveryChannel() throws {
        let six = try metrics(
    ...
    private func everyState() throws -> [SignalLevelMetricsPresentation] {
        [
            .loading, .metrics(try monoMetrics()), .metrics(try zeroFrameMetrics()), .absent,
            .failed(message: "The signal level metrics for this file could not be produced."),
        ]
    }
    ...
    @Test func metricsProduceNoSectionLevelTextOnlyRows() throws {
        #expect(SignalLevelMetricsCopy.text(for: .metrics(try monoMetrics())) == nil)
    }
    ...
    @Test func noSignalLevelTextCharacterisesTheSignal() throws {
        ...
        texts += allRowText(try monoMetrics()) + allRowText(try stereoMetrics()) + allRowText(try zeroFrameMetrics()) + allRowText(try silentMetrics())
        for text in texts {
    ...
    @Test func noSignalLevelTextLeaksAnInternalIdentifier() throws {
        let internals = [
        ...
        var texts = allStateText(.loading) + allStateText(.absent)
            + allStateText(.failed(message: "The signal level metrics for this file could not be produced."))
        texts += allRowText(try monoMetrics()) + allRowText(try stereoMetrics())
        for text in texts {
    ...
    @Test func noSignalLevelTextShowsAMathematicalInfinity() throws {
        for metrics in [try monoMetrics(), try stereoMetrics(), try zeroFrameMetrics(), try silentMetrics()] {
    ...
    @Test func everyFlowStateMapsToItsPresentation() throws {
        let metrics = try monoMetrics()
