...
struct SpectrogramFlowStateTests {
    ...
    private func settle(spectrogram outcome: SpectrogramOutcome) async throws -> InspectionPresentation {
        ...
        let shown = try #require(presentation(of: model))
        action.finish(.inspected(action.report, analyses: InspectionAnalyses(waveform: .unavailable, spectrogram: outcome, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
        await running.value
        return shown
    }
    ...
    func cancelledIsNotVisible() async throws {
        ...
        action.finish(.inspected(action.report, analyses: InspectionAnalyses(waveform: .unavailable, spectrogram: .cancelled, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
        await running.value
    }
    ...
    func theWaveformCanArriveFirst() async throws {
        ...
        action.finish(.inspected(action.report, analyses: InspectionAnalyses(waveform: .available(try envelope()), spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
        await running.value
    }
    ...
    func theSpectrogramCanArriveFirst() async throws {
        ...
        action.finish(.inspected(action.report, analyses: InspectionAnalyses(waveform: .unavailable, spectrogram: .available(try spectrogram()), signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
        await running.value
    }
    ...
    func failuresDoNotCross() async throws {
        ...
        action.finish(.inspected(
            action.report, analyses: InspectionAnalyses(waveform: .available(try envelope()), spectrogram: .failed(message: "no model"),
    ...
    func aStaleSpectrogramIsDropped() async throws {
        ...
        first.finish(.inspected(first.report, analyses: InspectionAnalyses(waveform: .available(try envelope()), spectrogram: .available(try spectrogram()), signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
    ...
    func aSelectionAfterTheReportIsAccepted() async throws {
        ...
        second.finish(.inspected(second.report, analyses: InspectionAnalyses(waveform: .unavailable, spectrogram: .unavailable, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
        await secondRun.value
        first.finish(.inspected(first.report, analyses: InspectionAnalyses(waveform: .cancelled, spectrogram: .cancelled, signalLevelMetrics: .unavailable, truePeak: .unavailable, loudness: .unavailable, significantBandwidth: .unavailable)))
        await firstRun.value
