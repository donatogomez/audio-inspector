...
- [x] 3.5 Map the conditional capabilities: `bitDepth` (PCM/lossless `available`, lossy `unsupported`, never formula-inferred), and the two bitrates kept separate — `declaredBitrate` (directly declared or `unavailable`, no self-computation) and `estimatedBitrate` (**always `uncertain`** with a `reason`). `bitDepth` from the reused ASBD `mBitsPerChannel` (never byte/packet-derived): Linear PCM (`kAudioFormatLinearPCM`) with `>0` → `available`; PCM with `0` → `unavailable`; **any non-PCM → `unavailable`** (robust lossy-vs-lossless-compressed classification needs a codec table ADR-0012 forbids and the spike never validated — so `unsupported` is intentionally not emitted here to avoid false precision; the lossy→`unsupported` split is deferred to compressed-format validation). `declaredBitrate` → `unavailable` (no direct source in AVFoundation per spike 0031/G; no self-computation, no AudioToolbox). `estimatedBitrate` → **always `uncertain`** (per criterion/matrix/design; never `unavailable`): carries the loaded `estimatedDataRate` as the tentative value only when finite, `>0`, and exactly an `Int` (no rounding), else `uncertain` with no value (spike 0031/G observed `0` for PCM). No fileSize/duration self-computation.
