...
- [x] 3.4 Map `container`, `duration`, and `codec` with their uncertainty policies (direct recognition → `available`; UTI/extension-only or estimate/discrepancy → `uncertain`; `codec` emits a stable non-localized token). `container` from `URLResourceValues.contentType` (a `UTType`): a resolved type → `uncertain` carrying the non-localized UTI identifier (no direct-recognition source exists — spike 0031/F — so never `available`), no type → `unavailable`. `duration` from the already-loaded `CMTime`: positive finite numeric → `available`; non-positive → `uncertain`; `nil`/invalid/indefinite/non-numeric → `unavailable` (errored→`failed` refinement is 3.6). `codec` from the reused ASBD `mFormatID` → a stable big-endian FourCC token (ASCII `0x21–0x7E`, else fixed-width hex; spike 0031/C), `available` for a non-zero id, else `unavailable`. Discrepancy reconciliation and the failed/error taxonomy stay with 3.6.
